<div class="bg-white  p15 no-border m0 rounded-bottom">
    ...
    <?php if ($invoice_info->subscription_id) { ?>
        <span class="ml15">
            <?php
            echo app_lang("created_from") . ": ";
            echo anchor(get_uri("subscriptions/view/" . $invoice_info->subscription_id), get_subscription_id($invoice_info->subscription_id));
            ?>
        </span>
    <?php } ?>
    ...
    <?php if ($invoice_info->estimate_id && $login_user->is_admin) { ?>
        <span class="ml15"><?php echo app_lang("created_from") . ": " . anchor(get_uri("estimates/view/" . $invoice_info->estimate_id), get_estimate_id($invoice_info->estimate_id)) ?></span>
    <?php } ?>
